...
export class DB {
	...
	addBook(book: CreateBookRequest.CreateBookData) {
		...
		if (this.getBook(id))
			throw new Error(
				`Book "${book.getTitle()}" by "${this.getAuthor(
					book.getAuthor()
	...
	updateBook(bookId: string, updateData: UpdateBookRequest.UpdateBookData) {
	...
	private removeEmptyKeys(
		obj: UpdateAuthorRequest.UpdateAuthorData | UpdateBookRequest.UpdateBookData
	) {
